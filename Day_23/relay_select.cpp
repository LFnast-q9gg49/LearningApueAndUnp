...
int relay(int fd_1, int fd_2){
    ...
    while(fsm12.state != STATE_T || fsm21.state != STATE_T){
        ...
        if (fsm12.state < STATE_AUTO || fsm21.state < STATE_AUTO) {
            if (select(nfds, &rset, &wset, nullptr, nullptr) < 0) {
                if (errno == EINTR) {
                    continue;
                }
                perror("select");
        ...
        if (FD_ISSET(fd_1, &rset) || FD_ISSET(fd_2, &wset) || fsm12.state > STATE_AUTO){
            fsm_driver(&fsm12);
        }
        if (FD_ISSET(fd_2, &rset) || FD_ISSET(fd_1, &wset) || fsm21.state > STATE_AUTO) {
            fsm_driver(&fsm21);
        }
    }
