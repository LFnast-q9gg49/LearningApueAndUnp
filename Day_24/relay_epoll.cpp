...
int relay(int fd_1, int fd_2){
    ...
    ev.data.fd = fd_1;
    ev.events = 0;
    epoll_ctl(epfd, EPOLL_CTL_ADD, fd_1, &ev);

    ev.data.fd = fd_2;
    ev.events = 0;
    epoll_ctl(epfd, EPOLL_CTL_ADD, fd_2, &ev);
    ...
    while(fsm12.state != STATE_T || fsm21.state != STATE_T){
        ...
        epoll_ctl(epfd, EPOLL_CTL_MOD, fd_1, &ev);
        ...
        // get result
        if (ev.data.fd == fd_1 && ev.events & EPOLLIN\
            || ev.data.fd == fd_2 && ev.events & EPOLLOUT\
            || fsm12.state > STATE_AUTO){
            fsm_driver(&fsm12);
        }
        if (ev.data.fd == fd_2 && ev.events & EPOLLIN\
            || ev.data.fd == fd_1 && ev.events & EPOLLOUT\
